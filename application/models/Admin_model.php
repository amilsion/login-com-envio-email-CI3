...
class Admin_model extends CI_Model {
    ...
    function insert_user($postData){
        ...
        if($validate){
            ...
            $message = "Aqui estão os detalhes da sua conta:<br><br>Email: ".$postData['email']."<br>Senha temporária: ".$password."<br>Por favor, altere sua senha após o login.<br><br> você pode fazer login em ".base_url().".";
            $subject = "Seja bem vindo ao nosso site"; // NAO PODE TER ACENTOS
            $this->send_email($message,$subject,$postData['email']);

            $module = "Gerenciamento de Usuários";
            $activity = "adicionado usuário(a) "."<b>".$postData['email']."</b>";
    ...
    function update_user_details($postData){
        ...
        if($validate){
            ...
            $record = "(".$oldData[0]['email']." para ".$postData['email'].", ".$oldData[0]['name']." para ".$postData['name'].",".$oldData[0]['role']." para ".$postData['role'].")";

            $module = "Gerenciamento de Usuários";
            $activity = "atualizado os detalhes do usuário "."<b>".$oldData[0]['email']."<b> ".$record;
            $this->insert_log($activity, $module);
    ...
    function deactivate_user($email,$id){
        ...
        $module = "Gerenciamento de Usuários";
        $activity = "Usuário(a) excluído "."<b>".$email."</b>";
        $this->insert_log($activity, $module);
        return array('status' => 'success', 'message' => '');
    ...
    function reset_user_password($email,$id){
        ...
        $message = "A senha da sua conta foi redefinida.<br><br>Email: ".$email."<br>Senha temporária: ".$password."<br>Por favor, mude sua senha após o login.<br><br> você pode fazer login em ".base_url().".";
        $subject = "Enviamos sua nova senha";
        $this->send_email($message,$subject,$email);

        $module = "Gerenciamento de Usuários";
        $activity = "usuário "."<b>".$email."</b>"." resetado a senha";
        $this->insert_log($activity, $module);
        return array('status' => 'success', 'message' => '');
    ...
    function send_email($message,$subject,$sendTo){
        ...
        try
        {
            ...
            $mail->Username = GUSER;  
            $mail->Password = GPWD;     
            $mail->SetFrom(GUSER, 'Administrador');
            //$mail->Subject = "DO NOT REPLY - ".$subject;
			$mail->Subject = "NAO RESPONDA - ".$subject;
            $mail->IsHTML(true);   
			$mail->CharSet = "utf-8";
            $mail->WordWrap = 0;
            ...
            $thanks = "<br><br><i>Este é um e-mail automático, por favor, não responda.</i><br/><br/>Obrigado,<br/>Administrador<br/><br/>";
